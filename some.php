	...
	<div class="container-fluid">
		...
		if(isset($_POST['single'])) {
			...
			<script type="text/javascript">
				$(function() {
					...
					var winnerImageName = '';
					var winnerImage = <?php echo json_encode($imageArr); ?>;
					...
					$('#start_single').click(function() {
						if(winnerImageName != '') {
				            ...
				            $('.single_number_' + winnerImageName).attr('src', winnerImage[winnerImageName - 1]['sname']);
				        }
						...
						setTimeout(function() {
				            clearInterval(randomImg);
				       		$('.single_number_' + imgName).css('display', 'none');
				            $('.single_number_' + winnerImageName).attr('src', winnerImage[winnerImageName - 1]['bname']);
		...
		<div id="buttons" class="row">
			<form role="form" method="POST" action="" id="start_form">
				<div class="checkbox-inline"><label><input type="checkbox" value="male" name="sex[]" checked /> Male</label></div>
				<div class="checkbox-inline"><label><input type="checkbox" value="female" name="sex[]" checked /> Female</label></div>
				<br />
		...
		<div id="winner_div" class="row">
			Ընտրիր նկարը, եթե ուզում ես կոնկրետ մարդ հաղթի, կամ էլ կարաս չնտրես, պատահական կլնի:
			<form method="POST" onsubmit="return process();" target="_blank">
				<div class="img_to_win">
					<?php
					if(isset($_POST['single'])) {
						foreach($images as $position => $image) {
						?>
						<div class="radio-inline"><label><img id="<?php echo $image['id']; ?>" onclick="bordercho(this.id);" src="<?php echo $image['bname']; ?>" height="128px"><input type="radio" value="<?php echo $image['id']; ?>" name="winner" /></label></div>
						<?php
						}
					}
					?>
			...
			<?php
			if(isset($_POST['win'])) {
				$urlHash .= '&winnerId=' . $_POST['winner'];
				$urlHash = openssl_encrypt($urlHash, 'method', 'password');
			}
			?>
			<script type="text/javascript">
			function process() {
				window.open("http://localhost?starting=true", '_blank', 'width=800,height=600');
				return false;
			}
			</script>
		</div>
